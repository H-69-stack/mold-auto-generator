...
v34 新增（快速检测 + 几何状态缓存 + 局部优先修复）：
  ...
  ⑤ 薄鳍 z 上限取【薄鳍顶面与产品上表面较高者】：薄鳍高于产品上表面时
   ...
   ⑧ 凹模挖空（v41 重写，用户要求把产品与基座顶面之间被包裹的间隙也挖掉）：
    ...
    ⑩ 凹模型腔侧壁精确贴合产品外侧 + 顶部封闭空间完全掏空（用户反馈：产品底部
       圆盘/加强筋挖去都正确，但型腔比产品外轮廓大一圈；且产品上表面与基座上顶面
       围成的封闭空间里不应有任何实体）：
       - 原因：① 顶部安装间隙用【产品 bbox 矩形外轮廓柱】挖，型腔侧壁变成矩形框，
         比产品真实外轮廓大一圈；② 若用【朝上外表面外轮廓柱】逐个切除，外轮廓
         缺口/凹坑处会漏挖，产品上表面与基座上顶面围成的封闭空间残留基座材料。
       - 修复：
         ① 型腔改为 Cut(基座, 产品)（侧壁=产品外侧形状，精确贴合）；
         ② 顶部安装间隙用【各朝上外表面的 bbox 矩形】作柱脚（覆盖完整投影，
            不因外轮廓缺口/凹坑漏挖），柱从主板面高度向下 0.5mm 重叠处拉伸到
            型腔开口面，逐个切除 —— 产品上表面以上、产品轮廓以内的基座材料
            全部掏空（封闭空间为空），转角凹槽（产品无料区）的模具壁保留；
         ③ 布尔产生的孔洞柱塞/悬浮碎片由 _consolidate_hollow_result 清理。
         实测 YA-1131 / YA-1238：型腔侧壁贴合产品外轮廓 0mm、产品上表面以上的
         封闭空间残留 = 0、产品 100% 放入型腔、Common(型腔,产品)=0、1 实体 1 壳、
         输出 = compound [型腔, 产品] 两个独立结构体、底部圆盘/加强筋保持正确。
...
class MoldFixer:
    ...
    def _hollow_cavity_solid3d(self, full_base, product, z_parting, clearance=0.0):
        ...
        # ---- 2. 顶部安装间隙：挖空【产品上表面以上、产品轮廓以内】的基座材料 ----
        #      产品上表面与基座上顶面（分模面）围成的封闭空间必须完全为空。
        #      用各朝上外表面的【bbox 矩形】作柱脚（覆盖完整投影，不因外轮廓
        #      缺口/凹坑漏挖），柱从主板面高度向上拉伸到型腔开口面；柱底向下
        #      0.5mm 重叠避免与产品上表面贴合处的布尔脆断；碎片交给 hollow()
        #      的 _consolidate_hollow_result 清理。
        main_z = None
        _best_area = 0.0
        exp = TopExp_Explorer(product, TopAbs_FACE)
        while exp.More():
            ...
            _, n = info
            if n.Z() > 0.05:
                _a = MoldFixer._face_area(f)
                if _a > _best_area:
                    _best_area = _a
                    main_z = MoldFixer._bbox_tuple(f)[5]
            exp.Next()
        cols = []
        if main_z is not None:
            ...
            while exp.More():
                ...
                try:
                    fbb = MoldFixer._bbox_tuple(f)
                    w, d = fbb[3] - fbb[0], fbb[4] - fbb[1]
                    if w < 3.0 or d < 3.0:
                        exp.Next()
                        continue  # 极小面（角部过渡），忽略
                    z0 = main_z - 0.5
                    h = (base_top + 0.001) - z0
                    if h <= 0.05:
                        exp.Next()
                    ...
                    col = BRepPrimAPI_MakeBox(
                        gp_Ax2(gp_Pnt(fbb[0], fbb[1], z0), gp_Dir(0, 0, 1)),
                        w, d, h).Shape()
                    if MoldFixer._solid_volume(col) > 1.0:
                        cols.append((z0, -w * d, col))
                except Exception:
                    pass
                exp.Next()
        cols.sort()   # 最深（z 最小）优先，保持主体连续
        n_ok = 0
        ...
        if n_ok:
            print(f"[MoldFix]   顶部安装间隙切除 {n_ok}/{len(cols)} 个朝上特征柱"
                  f"（产品上表面以上、轮廓以内的基座材料全部掏空，侧壁贴合产品外侧）")
        else:
